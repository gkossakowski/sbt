...
class NameHashFilteredDependencies(
	...
	def apply(to: File): Set[File] = {
		...
		cachedResults.getOrElseUpdate(to, filteredDependencies(to, dependent))
	}

	private def filteredDependencies(to: File, dependent: Set[File]) = allModifiedNames.get(to) match {
		case None =>
